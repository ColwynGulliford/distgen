    ...
    def construct_species_data(self):
        ...
        extra_species = {
            ...
            "H2+": {
                ...
                "mc2": 2 * self["proton mass energy equivalent in MeV"].to("eV")
                + self["electron mass energy equivalent in MeV"].to("eV"),
                #'g_factor': +self['electron g factor']
            },
        }
        ...
        for species in self._supported_species:
            ...
            if species in extra_species:
                ...
                self._species_data[species] = {
                    "charge": charges[species] * self["elementary charge"],
    ...
    def species_in_scipy(self, key):
        return (
            len(
                [
                    res
                    for res in scipy.constants.find(f"{key} mass")
                    if res == f"{key} mass"
                ]
            )
            == 1
