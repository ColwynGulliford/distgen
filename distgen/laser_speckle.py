...
def generate_laser_speckle_fft(image_size, speckle_size, random_seed=None):
    ...
    speckle_pattern = (
        (speckle_pattern - speckle_pattern.min())
        / (speckle_pattern.max() - speckle_pattern.min())
        * 255
    )
...
def generate_speckle_pattern_with_filter(image_size, sigma, random_seed=None):
    ...
    speckle_pattern_normalized = (speckle_pattern - np.min(speckle_pattern)) / (
        np.max(speckle_pattern) - np.min(speckle_pattern)
    )
