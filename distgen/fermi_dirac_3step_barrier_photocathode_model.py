...
def fermi_dirac_3step_barrier_pdf_int(
    px, py, pz, fermi_energy, temp, photon_energy, workfun
):
...
def ds_transform(px, py, pz, fermi_energy, photon_energy, workfun):
    ...
    a = np.sqrt(
        1
        - photon_energy
        / ((cpx**2 + cpy**2 + cpz**2) / 2 / MC2 + fermi_energy + workfun)
    )
...
def ds_jacobian_factor(px, py, pz, fermi_energy, photon_energy, workfun):
    ...
    num = cpz * np.sqrt(
        cpx**2 + cpy**2 + cpz**2 + 2 * MC2 * (fermi_energy + workfun - photon_energy)
    )
    den = np.sqrt(
        (cpz**2 + 2 * MC2 * (fermi_energy + workfun))
        * (cpx**2 + cpy**2 + cpz**2 + 2 * MC2 * (fermi_energy + workfun))
    )
...
def fermi_dirac_3step_barrier_pdf(
    px, py, pz, photon_energy, workfun, temp, fermi_energy
):
    """
    ...
    return a * fermi_dirac_3step_barrier_pdf_int(
...
def fermi_dirac_3step_barrier_pdf_bounds(
...
def fermi_dirac_3step_barrier_pdf_bounds_quantity(
    photon_energy, workfun, temp, fermi_energy, n_tails=4
):
    bounds = fermi_dirac_3step_barrier_pdf_bounds(
        photon_energy, workfun, temp, fermi_energy, n_tails=n_tails
    )
    return [bounds[0], bounds[1], (0 * bounds[2][1], bounds[2][1])]


def fermi_dirac_3step_barrier_pdf_spherical(
    p, theta, photon_energy, workfun, temp, fermi_energy
):
    return fermi_dirac_3step_barrier_pdf(
        p * np.sin(theta),
        0.0 * p,
        p * np.cos(theta),
        photon_energy,
...
def fermi_dirac_3step_barrier_pdf_bounds_spherical(
    photon_energy, workfun, temp, fermi_energy, n_tails=4
):
    (_, a), _, _ = fermi_dirac_3step_barrier_pdf_bounds(
        photon_energy, workfun, temp, fermi_energy, n_tails=n_tails
    )
